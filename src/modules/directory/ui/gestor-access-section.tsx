...
import { resendGestorAccessEmailAction } from '@/src/modules/directory/application/directory-mutations'
...
type GestorAccessSectionProps = {
  gestor: GestorRecord
}

export function GestorAccessSection({ gestor }: GestorAccessSectionProps) {
  const copy = equipo.form.gestorAccessSection
  const [pending, startTransition] = useTransition()
  const [confirmOpen, setConfirmOpen] = useState(false)
  const isActive = gestor.status === 'active'

  function run() {
    startTransition(async () => {
      const result = await resendGestorAccessEmailAction(gestor.id)
      if (result.ok) {
  ...
  return (
    <>
      <section
        aria-labelledby="gestor-access-section-title"
        className="mt-8 rounded-lg border border-border bg-muted/30 px-4 py-5"
      >
        <h2
          id="gestor-access-section-title"
