...
import { deleteGestorAction } from '@/src/modules/directory/application/directory-mutations'
import type { GestorRecord } from '@/src/modules/directory/domain/types'

type GestorDangerZoneProps = {
  gestor: GestorRecord
  onDeleted: () => void
}

export function GestorDangerZone({ gestor, onDeleted }: GestorDangerZoneProps) {
  const copy = equipo.form.gestorDangerZone
  ...
  const emailMatches =
    emailConfirm.trim().toLowerCase() === gestor.email.trim().toLowerCase()
  ...
  function handleDelete() {
    startTransition(async () => {
      const result = await deleteGestorAction(gestor.id)
      if (result.ok) {
      ...
      if (result.error === 'forbidden') {
        toast.error(result.message ?? formCopy.errors.forbidden)
        return
      }
      ...
      <Dialog open={confirmOpen} onOpenChange={handleConfirmOpenChange}>
        <DialogContent className="max-w-md">
          ...
          <div className="rounded-md bg-muted/60 px-3 py-2 text-sm">
            <p className="font-medium text-foreground">{gestor.name}</p>
          ...
          <div className="flex flex-col gap-2">
            <label
              htmlFor="gestor-delete-email-confirm"
              className="text-sm font-medium text-foreground"
            >
              {copy.confirmEmailLabel}
            </label>
            <Input
              id="gestor-delete-email-confirm"
              type="email"
              value={emailConfirm}
